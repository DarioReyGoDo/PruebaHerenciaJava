...
import juego.Personaje.Raza;

public class Main implements ICreableEstadisticas{
	/*
	...
	public static void main(String[] args) {
		...
		while (opcion != 7) {
			...
			case 6:
				...
				Arrays.sort(Main.personaje, Comparator.nullsLast(new PuntosVidaComparator()));
				Main.mostrarListaPersonajes(Main.personaje);
				
				System.out.println("Orden invertido.");
				Arrays.sort(Main.personaje, Comparator.nullsLast(new PuntosVidaComparatorInverso()));
				Main.mostrarListaPersonajes(Main.personaje);
	...
	public static void mostrarListaPersonajes(Personaje[] personaje) {
		System.out.println("Comienzo de lista de Personajes:");
		for (Personaje p : personaje) {
			if (p != null) {
				System.out.println(p);
			}
		}
		System.out.println("Fin de lista de Personajes.");
	}
	...
	@Override
	public double minimo() throws NoPersonajeException {
		double minVida = Double.MAX_VALUE;
		Main.hayPersonajes();
		for (Personaje p: Main.personaje) {
			if (p != null) {
				minVida = minVida > p.getPuntos_vida_actuales() ? p.getPuntos_vida_actuales() : minVida;
			}
		}
		return minVida;
	}
	...
	@Override
	public double maximo() throws NoPersonajeException {
		double maxVida = Double.MIN_VALUE;
		Main.hayPersonajes();
		for (Personaje p: Main.personaje) {
			if (p != null) {
				maxVida = maxVida < p.getPuntos_vida_actuales() ? p.getPuntos_vida_actuales() : maxVida;
			}
		}
		return maxVida;
	}
	...
	@Override
	public double media() throws NoPersonajeException {
		double media = 0;
		int counter = 0;
		for (Personaje p : Main.personaje) {
			if (p != null) {
				media += p.getPuntos_vida_actuales();
				counter++;
			}
		}
		if (counter == 0) {
			throw new NoPersonajeException("No hay ningún personaje creado.");
		}
		media /= counter;
		return media;
	}

	private static void hayPersonajes() throws NoPersonajeException {
		int counter = 0;
		for (Personaje p5: Main.personaje) {
			if (p5 != null) {
				counter++;
